...
class MinPlatforms {  // Greedy Algorithm
    static int findPlatform(int arr[], int dep[], int n) {
        ...
        int plat_needed = 1, maxPlatforms = 1; //assigned 1 becoz, we are assuming 1st train is reached ie, 0th index train
        int i = 1, j = 0;  //i=1 becoz, we already assumed 1st train reached platform 1 as above
        ...
        while (i < n && j < n) {
            ...
            if (plat_needed > maxPlatforms)  //find max platforms needed
                maxPlatforms = plat_needed;
        }
        return maxPlatforms;
    }
